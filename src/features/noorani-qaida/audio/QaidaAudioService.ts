...
class QaidaAudioService {
  ...
  private async playSpeech(text: string, rate: number) {
    // Speak the exact Arabic shown on screen (no letter→madd rewrite — that made
    // رَ/دَ/زَ sound like "run/dun/zun" on many browser voices).
    await speakArabic(text.trim(), rate, 1.02);
  }
